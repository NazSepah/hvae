...
from unet_utils import resize_down2d
...
def ce_loss(logits, labels, sample_weights, mask=None, top_k_percentage=None,
            deterministic=False):
...
def ce_loss_h(latents, labels, sample_weights, mask=None):
    """Computes the cross-entropy loss.
    ...
    for i in range(1, 8):
        ...
        labels = resize_down2d(labels, scale)

    loss = {'mean': [], 'sum': [], 'mask': []}
    for i in range(len(latents)):
        y_flat = tf.reshape(latents[i], (-1, num_classes), name='reshape_y')
        t_flat = tf.reshape(labels_level[i + 1], (-1, num_classes), name='reshape_t')

        if mask is None:
            mask_i = tf.ones(shape=(t_flat.shape.as_list()[0],))
        else:
            assert mask.shape.as_list()[:3] == labels.shape.as_list()[:3], \
                'The loss mask shape differs from the target shape: {} vs. {}.'.format(
                    mask.shape.as_list(), labels.shape.as_list()[:3])
            mask_i = tf.reshape(mask, (-1,), name='reshape_mask')
        ...
        batch_size = labels.shape.as_list()[0]
        xe = tf.reshape(xe, shape=(batch_size, -1))
        mask_i = tf.reshape(mask_i, shape=(batch_size, -1))

        ce_sum_per_instance = tf.reduce_sum(mask_i * xe, axis=1)
        ce_sum = tf.reduce_mean(ce_sum_per_instance, axis=0)
        ce_mean = tf.reduce_sum(mask_i * xe) / tf.reduce_sum(mask_i)

        loss['mean'].append(ce_mean)
        loss['sum'].append(ce_sum)
        loss['mask'].append(mask_i)
    return loss
